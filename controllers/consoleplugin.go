...
func getPluginPodSpec(imagePullPolicy corev1.PullPolicy) corev1.PodSpec {
...
func pluginDeployment(imagePullPolicy corev1.PullPolicy) *appsv1.Deployment {
	podSpec := getPluginPodSpec(imagePullPolicy)
...
func (r *ReconcileGitopsService) reconcileDeployment(cr *pipelinesv1alpha1.GitopsService, request reconcile.Request) (reconcile.Result, error) {
	...
	imagePullPolicy := common.GetImagePullPolicy(cr.Spec.ImagePullPolicy)
	newPluginDeployment := pluginDeployment(imagePullPolicy)
