...
func TestGetImagePullPolicy(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "Default to Always when no config",
			crPolicy:       "",
			envValue:       "",
			expectedPolicy: corev1.PullAlways,
		},
		{
			name:           "Invalid env value defaults to Always",
			crPolicy:       "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.envValue != "" {
				os.Setenv(ImagePullPolicyEnvVar, tt.envValue)
			} else {
				os.Unsetenv(ImagePullPolicyEnvVar)
			}
			defer os.Unsetenv(ImagePullPolicyEnvVar)
